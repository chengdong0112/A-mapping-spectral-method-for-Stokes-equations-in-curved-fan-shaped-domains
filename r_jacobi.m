...
if nargin<2, a=0; end;  if nargin<3, b=a; end%默认参数，若未指定a，b则默认值为0，雅可比多项式退化为勒让德多项式
if((N<=0)||(a<=-1)||(b<=-1)); error('parameter(s) out of range'); end
nu=(b-a)/(a+b+2);
mu=2^(a+b+1)*gamma(a+1)*gamma(b+1)/gamma(a+b+2);%首项递推系数nu，首项范数mu
if N==1, ab=[nu mu]; return, end 
N=N-1; n=1:N; nab=2*n+a+b;%nab为计算的中间变量
A=[nu (b^2-a^2)*ones(1,N)./(nab.*(nab+2))];%计算α系数的数组A
n=2:N; nab=nab(n);
B1=4*(a+1)*(b+1)/((a+b+2)^2*(a+b+3));%计算β1系数
B=4*(n+a).*(n+b).*n.*(n+a+b)./((nab.^2).*(nab+1).*(nab-1));%计算大于2的通用项
ab=[A' [mu; B1; B']];%合并得到ab
