...
N0=size(ab,1); if N0<N, error('input array ab too short'), end%检查输入数组 ab 是否包含至少 N 行。
J=zeros(N);
...
[V,D]=eig(J);%计算J 的特征值 D（对角矩阵）和特征向量矩阵 V
[D,I]=sort(diag(D));%按升序排列特征值，并同步调整特征向量顺序
V=V(:,I);
xw=[D ab(1,2)*V(1,:)'.^2];%计算权重
